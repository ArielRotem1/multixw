...
export function Clue({ clue, word, hashedAnswer, dir, isSolved, grid, currCell, playerID, getCompletedWordsIDs, markWordCellsAsCorrect, numberOfFreeHints, setNumberOfFreeHints }) {
    ...
    async function getHint(clue, letterIndex, hashedAnswer) {
        const response = await fetch("api/v1/hint", {
            ...
            body: JSON.stringify({ clue, letterIndex, hashedAnswer, numberOfCluesAvaliable: numberOfFreeHints })
        });
    ...
    async function handleRequestHint() {
        ...
        let chosenLetterIndexToReveal = indicisToComplete[Math.floor(Math.random() * indicisToComplete.length)];
        let singleLetterToReveal = await getHint(clue, chosenLetterIndexToReveal, hashedAnswer);
        ...
        let cellToReveal = document.querySelector(`[data-pos="${row + " " + col}"]`);
        ...
        getCompletedWordsIDs(row, col, cellToReveal).then(completedWordsIDs => markWordCellsAsCorrect(completedWordsIDs));
        currCell.focus();
    ...
    return (
        <>
            <Overlay
                ...
                show={showToolTip}
                placement="left">
                <Tooltip style={{ width: "20vmin" }} onClick={() => {
                    handleRequestHint();
                    setShowToolTip(prev => !prev);
                }}>
                    {numberOfFreeHints > 0 ? "Click again to get hint" : "This hint would cost 5 coins"}
                </Tooltip>
            </Overlay>
            <Container className="d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex justify-content-center align-items-center">
                    ...
                    {dir === "ACROSS" ?
                        <svg className="mt-2" width="4vmin" height="4vmin" fill="currentColor" viewBox="0 0 16 16">
                    ...
                    <div id="clue" style={{ "fontSize": `${(4.5 - clue.length * 0.03)}vmin` }} className="mt-1 text-center">{clue}</div>

                    {isSolved && (<svg id="solved-icon" width="4vmin" height="4.5vmin" fill="green" viewBox="0 0 16 16">
                ...
                {isClueAvaliable ?
                    ...
                    (<svg id="hint-icon" width="4vmin" height="3vmin" fill="black" viewBox="0 0 16 16">
                        <path d="M2 6a6 6 0 1 1 10.174 4.31c-.203.196-.359.4-.453.619l-.762 1.769A.5.5 0 0 1 10.5 13h-5a.5.5 0 0 1-.46-.302l-.761-1.77a1.964 1.964 0 0 0-.453-.618A5.984 5.984 0 0 1 2 6zm3 8.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1l-.224.447a1 1 0 0 1-.894.553H6.618a1 1 0 0 1-.894-.553L5.5 15a.5.5 0 0 1-.5-.5z" />
                    </svg>)
                }
            </Container>
