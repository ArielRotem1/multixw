...
export function SinglePlayer() {
    ...
    function startGame() {
        ...
        fetch("/api/v1/crosswordGame", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ gridSize, debug })
        })
    ...
    function markWordCellsAsCorrect(wordIDs) {
        ...
        if (wordIDs.down !== -1) {
            ...
            document.querySelectorAll(`[data-word_id_down="${wordIDs.down}"]`)
                .forEach(cell => {
        ...
        if (wordIDs.across !== -1) {
            ...
            document.querySelectorAll(`[data-word_id_across="${wordIDs.across}"]`)
                .forEach(cell => {
                    cell.classList.add("correct");
                    let [i, j] = cell.dataset.pos.split(" ").map((string) => parseInt(string));
                    grid[i][j].isCorrect = true;
    ...
    const progress = wordsAndCells.length === 0 ? 0 : ((solvedWordsIDs.size * 1.0) / wordsAndCells.length) * 100;
    ...
    return (
        <>
            ...
            {wordsAndCells.length !== 0 &&
                ...
                <WinMessage startGame={startGame} />}
