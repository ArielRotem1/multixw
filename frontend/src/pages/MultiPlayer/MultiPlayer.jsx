...
export function MultiPlayer() {
    ...
    useEffect(() => {
        ...
        //set variable to return client to home page on reload and remove socket from game waiting list
        window.onbeforeunload = () => {
            if(socket !== null && grid[0].length === 0) socket.emit("remove from game", undefined, numberOfPlayers, gridSize);

            window.localStorage.setItem("back to home", "true");
        }

        //remove socket from game waiting list
        window.onpopstate = () => {
            if(socket !== null) socket.emit("remove from game", undefined, numberOfPlayers, gridSize);
        }

        //return client to home page
        if(window.localStorage.getItem("back to home") === "true") {
            navigate("/");
            return;
        }
        ...
        const newSocket = new io("/");

        if (GAME_ID === undefined) newSocket.emit("search for game", numberOfPlayers, gridSize, debug);
        else newSocket.emit("join to specific game", GAME_ID, numberOfPlayers, gridSize, debug);
    ...
    useEffect(() => {
        ...
        //possible fix to selectWordAtGameStart
        setTimeout(() => {
            selectWordAtGameStart();
        }, 2000);

    }, [grid, socket]);

    async function selectWordAtGameStart() {
        ...
        if (cells.length > 0) {
            ...
            for (let cell of cells) {
                ...
                cellNumber++;
                if(cellNumber < playerID) continue;

                let isApproved = await selectCell(cell);
                if (isApproved) break;
                //if not approved, then change dir and try again
                else{
                    currDir = currDir === "ACROSS" ? "DOWN" : "ACROSS";
                    isApproved = await selectCell(cell);
                    if (isApproved) break;
                }
            }
        }
    ...
    if (grid[0].length === 0) {
        ...
        let message = (GAME_ID === undefined ? "Searching" : "Waiting") + " for players" + dots;

        return (
            <div className="mid-height text-center fade-in-text">
                <h1>{message}</h1>
                {GAME_ID !== undefined &&
                    <div>
        ...
        return (
            <h1 className="mid-height text-center fade-in-text">Ready to play!</h1>
        );
    }
